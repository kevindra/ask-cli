...
module.exports = class SkillInfrastructureController {
    ...
    updateSkillManifestWithDeployResult(rawDeployResult, callback) {
        const domains = ResourcesConfig.getInstance().getTargetDomains(this.profile);
        ...
        if (ResourcesConfig.getInstance().hasEventsEndpoint(this.profile)) {
            Manifest.getInstance().setEventEndpoint(rawDeployResult.default.endpoint.uri);
        }
